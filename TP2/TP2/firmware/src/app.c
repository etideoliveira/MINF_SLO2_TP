...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_INIT:
        {
            ...
            // Mise à jour de l'état de l'application vers l'attente
        ...
        case APP_STATE_SERVICE_TASKS:
        {
            ...
            static uint8_t sendCounter = ZERO;
            static uint8_t CommStatus = ZERO;
            ...
            if (sendCounter == CINQ) {
                ...
                if (CommStatus == ZERO) // local ?
                {
                    SendMessage(&PWMData); // local
                } else {
                    SendMessage(&PWMDataToSend); // remote
                }
                sendCounter = ZERO; //Remet le compteur à 0
            }
            else {
                sendCounter++; //Incrémente le compteur
            }
            // Mise à jour de l'état de l'application vers l'attente
            APP_UpdateState(APP_STATE_WAIT);
            break;
        ...
        case APP_STATE_WAIT:
        {
            // Attente sans action spécifique
            break;
        }

        default:
        {
            // Cas par défaut si l'état est inconnu
            break;
        }
    }
}
...
void APP_UpdateState(APP_STATES newState) {
    appData.state = newState; // Modification de l'état courant
}
/*******************************************************************************
 End of File
 */
