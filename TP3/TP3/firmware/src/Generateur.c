...
#include "Mc32NVMUtil.h"         // Inclusion du fichier d'en-tête pour l'utilisation de la mémoire non volatile
...
void GENSIG_Initialize(S_ParamGen *pParam) {
    ...
    if (valeursParamGen.Magic == MAGIC) {
        *pParam = valeursParamGen; // Sauvegarde des valeurs récupérées
    } else {
        pParam->Forme = SignalSinus; // Forme du signal par défaut : Sinus
...
void GENSIG_UpdateSignal(S_ParamGen *pParam) {
    ...
    for (nbEchantillon = 0; nbEchantillon < MAX_ECH; nbEchantillon++) {
        switch (pParam->Forme) {
            case SignalSinus: // Cas du signal sinus
                valBrute = MOITIE_AMPLITUDE + offset + tableauSinus[nbEchantillon] * amplitude / DEUX;
                break;
            case SignalTriangle: // Cas du signal triangle
                ...
                break;
            case SignalDentDeScie: // Cas du signal dent de scie
                valBrute = MOITIE_AMPLITUDE + (offset + ((nbEchantillon - MOITIE_ECH) * amplitude_en_mV));
                break;
            case SignalCarre: // Cas du signal carré
                if ((MAX_ECH / DEUX) > nbEchantillon) {
                    valBrute = MOITIE_AMPLITUDE + offset + amplitude;
                } else {
                    valBrute = MOITIE_AMPLITUDE + offset - amplitude;
                }
                break;
