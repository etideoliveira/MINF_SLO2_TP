...
#include "Mc32NVMUtil.h"              // Inclusion du fichier d'en-tête pour l'utilisation de la mémoire non volatile (NVM)
    ...
    switch (MENU_DATA) {
        ...
        case MENU_STATE_FORME:
            ...
            if (Pec12IsPlus()) {
                ...
                MENU_DATA = MENU_STATE_SAVE; // Passe à l'état de sauvegarde
                LCD_ClearScreen(); // Efface l'écran
            }
            break;
        ...
        case MENU_STATE_FORME_VALUE:
            ...
            if (Pec12IsOK()) {
                ...
                switch (CompteurForme) {
                    case 0:
                        pParam->Forme = SignalSinus; // Définit la forme sinus
                        break;
                    case 1:
                        pParam->Forme = SignalTriangle; // Définit la forme triangle
                        break;
                    case 2:
                        pParam->Forme = SignalDentDeScie; // Définit la forme dent de scie
                        break;
                    case 3:
                        pParam->Forme = SignalCarre; // Définit la forme carré
                        break;
                }
        ...
        case MENU_STATE_FREQUENCE_VALUE:
            ...
            if (Pec12IsOK()) {
                ...
                // On retourne au menu précédent
                MENU_DATA = MENU_STATE_FREQUENCE; // Retourne à l'état de fréquence
            }// Vérifie si le bouton de sauvegarde est pressé
            else if (PLIB_PORTS_PinGet(S_OK, PORT_CHANNEL_G, PORTS_BIT_POS_12) == 0) {
                MENU_DATA = MENU_STATE_SAVE; // Passe à l'état de sauvegarde
                LCD_ClearScreen(); // Efface l'écran
            }
        ...
        case MENU_STATE_SAVE:
            // Affichage spécifique pour la sauvegarde
            lcd_gotoxy(1, 2); // Positionne le curseur à la deuxième ligne
            printf_lcd("   Sauvegarde  ?"); // Demande à l'utilisateur de confirmer la sauvegarde
            lcd_gotoxy(1, 3); // Positionne le curseur à la troisième ligne
            printf_lcd("   (appuis long)"); // Indique que l'appui doit être long pour sauvegarder

            // Vérifie si le bouton de sauvegarde est pressé
            if (PLIB_PORTS_PinGet(S_OK, PORT_CHANNEL_G, PORTS_BIT_POS_12) == 0) {
                CompteurAppuis++; // Incrémente le compteur d'appuis
            } else {
                if (CompteurAppuis > 0) { // Si un appui a été détecté
                    if (CompteurAppuis >= DEUX_SECONDES) { // Vérifie si l'appui est long
                        // Sauvegarde dans la mémoire flash
                        NVM_WriteBlock((uint32_t*) pParam, sizeof (S_ParamGen)); // Sauvegarde les paramètres
                        lcd_gotoxy(1, 4); // Positionne le curseur à la quatrième ligne
                        printf_lcd("Sauvegarde OK"); // Indique que la sauvegarde a réussi
                        CompteurAppuis = 0; // Réinitialise le compteur d'appuis
                    } else {
                        // Retour au menu forme
                        MENU_DATA = MENU_STATE_FORME; // Retourne à l'état de forme
                        CompteurAppuis = 0; // Réinitialise le compteur d'appuis
                        // On met à jour le flag qui affiche le menu
                        firstTimeIn = 1; // Indique qu'il faut réinitialiser l'affichage
                    }
                }
            }
            break;
