...
void __ISR(_TIMER_1_VECTOR, ipl3AUTO) IntHandlerDrvTmrInstance0(void) {
    ...
    static uint32_t compteur3s = 0;
    static uint32_t compteur10cycle = 0;

    if (compteur3s >= ATTENTE3SEC) { //test si 3 secondes alors aller dans task
        if (compteur10cycle >= ATTENTE10CYCLES) {
